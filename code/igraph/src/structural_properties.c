...
int igraph_shortest_paths_stats(const igraph_t *graph, igraph_matrix_t *res,
  ...
  long int diameter_touched_edges = 0;
  ...
  if (stats && stats_names) {
    igraph_vector_push_back(stats, diameter_touched_edges);
...
int igraph_shortest_paths(const igraph_t *graph, igraph_matrix_t *res, 
			  const igraph_vs_t from, const igraph_vs_t to,
			  igraph_neimode_t mode) {
  return igraph_shortest_paths_stats(graph, res, NULL, NULL, from, to, mode);
}
...
int igraph_i_subgraph_create_from_scratch(const igraph_t *graph, 
  ...
  for (i = 0; i < no_of_nodes; i++) {
    long int new_vid = VECTOR(*my_vids_old2new)[i] - 1;
    if (new_vid < 0)
      continue;
    ...
    if (directed) {
      for (j = 0; j < n; j++) {
        ...
        from = VECTOR(*my_vids_old2new)[(long int)IGRAPH_FROM(graph, eid)];
        if (!from) continue;
        to = VECTOR(*my_vids_old2new)[(long int)IGRAPH_TO(graph, eid)];
        if (!to) continue;
