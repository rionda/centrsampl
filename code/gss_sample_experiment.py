#! /usr/bin/env python3
""" gss_sample_experiment.py
    
    Perform experiment to evaluate the Geisberger et al. (linear scaling)
    sampling algorithm to approximate the betweenness centrality of all
    vertices in a graph. The algorithm is run multiple times. We compute
    aggregate statistics about the runs and save the stats and all the results
...
def main():
    ...
    for i in range(args.runs):
        ...
        # Compute betweenness
        if args.samplesize:
            results.append(geisbergerss_sample.betweenness_sample_size(G,
                args.samplesize, False, args.timeout))
        else:
            results.append(geisbergerss_sample.betweenness(G, args.epsilon,
