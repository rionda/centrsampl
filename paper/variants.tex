...
can be stopped after [we reached][ EK: "reaching] the vertices that are $k$ ``hops'' far from $u$.
...
\begin{definition}[\citep{KourtellisASIT12}]\label{def:kpathbetweenness}
 ...
 $s$ and [extinguishing][ EK: Why not "stopping"? :P] after having touched (at most) $k+1$ vertices ($s$
...
each vertex $v\in V$, the range $R_v$ is the subset of $B$ containing [all and][ EK: remove?]
only the random walks from $B$ that have $v$ as internal vertex. It is easy to
see that $\VC(\range_G^{\mathrm{p},k})$ is at most $\lfloor\log_2(k-1)+1$ [ EK: you need the right floor. 
The one that matches the carpet :P ]
...
sample a simple random walk touching $\ell$ vertices[ EK: So you consider the sampled-source as touched? If not then 
the previous range should be $[1,k+1]$], while updating the estimated
...
lower} number $r$ of samples, [precisely][ EK: specifically]
\[
r=2n^{1-2\alpha}\left(\ln n+\frac{\lfloor\log_2(k-1)\rfloor+1}{2}\right)\enspace.
\]
[In conclusion,][ EK: When i read that I scrolled down to the conclusion. "In summary". Maybe we should also remove
the "will" words that are here and there. Irrelevant but I can not find the right place to add this comment.] we presented a tighter analysis of the algorithm
...
[ EK: Maybe we should mention that in the introduction/preliminaries too.]It is
