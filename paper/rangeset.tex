...
graph similar to that in Fig.\XXX . $G_d$ has \XXX top vertices, \XXX
bottom vertices, and \XXX middle vertices. For each top vertex $v$,
let $\mathsf{f}(v)$ be the corresponding bottom vertex, i.e., the
bottom vertex $u$ whose neighbors are the same middle vertices that
are neighbors of $v$. Among the middle vertices, the two with degree 1
are special and are called the \emph{end vertices} of $G_d$ and
denoted as $v_\mathrm{l}$ and $v_\mathrm{r}$, where the labels can be
arbitrarily assigned. We are now going to build a set $Q$ of $d$
shortest paths from $v_\mathrm{l}$ to $v_\mathrm{r}$ and show that it
is shattered by $\range_{G_d}$, therefore proving that
...
\begin{IEEEproof}
  ...
  $S=2^Q\setminus\{Q,\emptyset\}$. We can partition $S$ in two
  sets $A$ and $B$ such that $A\cap B=\emptyset$ and $A\cup
  B=Q$ in the following way: for each unordered pair $(s',s'')$ of
  elements in $S$ such that $s'\cap s''=\emptyset$ and $s'\cup s''=Q$
  we put $s'$ in $A$ and $s''$ in $B$. For each $s'\in A$, let
  $\mathsf{c}(s')$ be the element $s''$ of $B$ such that $s'\cap
  s''=\emptyset$ and $s'\cup s''=Q$. Take any one-to-one map
  $\mathsf{r}$ from the top vertices of $G_d$ to the elements of
  $A$. We can create another map $\mathsf{r}_\mathsf{f}$ that maps the
  element $\mathsf{c}(\mathsf{r}(v))$ of $B$ to the bottom vertex
  $\mathsf{f}(v)$ corresponding to $v$, for each top vertex $v$.
  A path $p\in Q$ passes through a top vertex $v$ if and only if
  $p\in\mathsf{r}(v)$. Analogously, $p$ passes through a bottom vertex
  $u$ if and only if $p\in\mathsf{r}_\mathsf{f}(u)$.

  It is easy to see that, if we combine $\mathsf{r}$ and
  $\mathsf{r}_\mathsf{f}$, we obtain a map $\mathsf{q}$ from $S$ to the set
  of top and bottom vertices of $G_d$. We now show that for each $s\in
  S$, $s=Q\cap\mathcal{T}_{\mathsf{q}(s)}$.
\end{IEEEproof}

\begin{corol}\label{lem:vcdimequal}
  $\VC(\range_{G_d})=d$.
\end{corol}
