...
the other. Not that since $|E|\ge 2$ and the size of a path is always at least
$2$, then the formula for $h$ is always well defined.

[ EK: I can not right now why it is better to use the H(G) formulation instead
of working directly with VD(G).]

[ MR: It allows for a (theoretically) tighter bound, or at least a better
characterization ]

[EK: The above sentence is a bit confusing to me. The way that $\{\mathsf{Int}(p_1),\dotsc,\mathsf{Int}(p_h)\}$  is written describes a set of sets
and not a set of paths. Maybe you want to use the union of Int()s.]

[MR: No, it's correct. A set of sets.]

[EK: Furthermore I can not see why d can not be less or equal to 2. 
If that is the case, it throws the h formula off.]

[MR: $d$ is the size of the path. The size of a path is always at least 2,
because it includes the extremes. E.g., if $(u,v) \in E$, the path from $u$ to
$v$ is $\{u,v\}$ and has size $2$. The only case when $H(G)$ may be less than 2
is that there is a single edge in the graph. I added clarification in the
paragraph.]
...
%\begin{IEEEproof}
  ...
  Due to fact $\mathsf{F}$, all paths in $Q_x$ must go through the same vertices
...
[ EK: Maybe we should write formally the connectivity 'cause it is not obvious. ]
By construction, $\VD(G_d)=2^d+1$, so that
$\lfloor\log_2(\VD(G_d)-2)\rfloor+1=d$. The $G_d$ concertina graph has total
$3(2^{d-1})$ vertices, that can be partitioned into three classes, \emph{top},
\emph{bottom}, and \emph{middle},
