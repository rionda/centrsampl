...
[EK: This holds for the entire section, in lemmas and theorems we have to specify if they refer to undirected graphs,
directed graphs or both. ]
We now define [a range set of the shortest paths][ EK: in the definition of range set you use the proposition on, thus i think it is more suitable to say
a range set ON the domain of shortest paths] of a graph $G=(V,E)$, and present 
a [strict][ EK: What's the difference between a strict and a tight?] upper bound to its VC-dimension. %We show that this upper bound is strict. 
...
$\{\mathsf{Int}(p_1),\dotsc,\mathsf{Int}(p_h)\}$ is an [anti-chain][ EK: You need to define anti-chains. This is about both undirected and directed graphs.].

[ EK: I can not right now why it is better to use the H(G) formulation instead of working directly with VD(G). 
The above sentence is a bit confusing to me. The way that $\{\mathsf{Int}(p_1),\dotsc,\mathsf{Int}(p_h)\}$  is written describes a set of sets
and not a set of paths. Maybe you want to use the union of Int()s. Furthermore I can not see why d can not be less or equal to 2. 
If that is the case, it throws the h formula off. Overall, what is the benefit of using this H(G) formulation?]
...
other. Given that $p$ [belongs to each $S_i$][ EK: "is a member of every $S_i$"], [then][ EK: unnecessary] $p$ must also belong to each
...
[ EK: Maybe we should make more clear visually, that we only talk about unique shortes paths here. Subsection? ]
\paragraph{Unique shortest paths}\label{sec:rangeunique}
In the [restricted][ EK: restricted doesn't seem to be the right word] case when the graph is undirected and
...
practical point of view, we [will][ EK: remove?] see in Sect.~\ref{sec:algo} that this result has an
...
%\begin{IEEEproof}
  ...
  together][ EK: up to where?] or %at a certain point 
  [they separate never to meet again at any other
  vertex $v\neq u$.][ EK: They can do both, they can meet once go on, and then separate never to meet again. 
  It might be clearer if we presented it as \~"their interection is a single path with size >=1".
  Basically what we want to say is that they can overlap only once and this can be either a node or a path.] This is easy to see: if they could separate at $u$ and then
  ...
  Assume now that $\VC(\range_G)>3$, then there must be [a set][ EK: "a graph such that, there exists a set"] 
  ...
  fact $\mathsf{F}$. ][ EK: How is this related? ]
  
  Let us then assume that both $x$ and $y$ exist.
  Let $Q_x=\mathcal{T}_x\cap Q$ and $Q_y=\mathcal{T}_y\cap Q$.
  [Because of fact][ EK: Due to fact ] $\mathsf{F}$, all paths in $Q_x$ must go through the same vertices
  ...
  shattered. ][ EK: Needs a bit more explanation. "Notice that $p^*$ and $p_1$ are distinct. Therefore if there was vertex 
   $v$ such that $\mathcal{T}_v\cap Q=\{p_1,p^*\}$, the paths $p_1$ and $p^*$ would meet in $w$ and $v$; which contradicts fact $F$."]
   
  
  [Suppose from now on that $Q_x\cup Q_y=Q$][ EK: "Suppose" looks like another case analysis go for "Therefore from now on we can assume that $Q_x\cup Q_y=Q$". I would write in the beginning of the proof that
  we will do a case analysis, and then use a "In case" phrase to mark clearly where is what. ]. 
  [ EK: "We proceed with a case analysis on the intersection of $Q_x$ and $Q_y$. In case..."]
  If $Q_x\cap Q_y=Q$, then
  ...
  shattered. [Suppose instead][ EK: "Therefore it must be..."] that [$Q_x\cap Q_y\neq Q$][ EK: again $\subset$ instead of $\neq$] and let $S=(Q_x\cap
  ...
  that $Q_x\cap Q_y\neq Q$, there must be a path [$p^*\in Q\setminus\{p_1\}$][ EK: go for double prime $p''$, you used $p^*$ already]
  ...
  such that $\mathcal{T}_v\cap Q=\{p_1,p^*\}$, hence $Q$ cannot be shattered. [ EK: I feel there is a better way to describe the previous case, the current description is a bit confusing]
  [Assume][ EK: Therefore we can assume that] now that $S=\emptyset$ and consider the case $Q_x=\{p_1,p_2,p_3\}$,
  ...
  $y$.][ EK: Not clear. Maybe substitute "there can not be a vertex $v$  between the end point of $p_1$ before $x$ and $w$"
  with \~"there can not be a vertex $v$ in the sub-path that starts from the predecessor of $x$(according to $p_1$) and $w$". 
  Let's do something similar for the next sentence too.] This implies that $Q$ can not be shattered.
...
a directed graph [ EK: "for which there is a set of four unique shortest paths, that can be shattered"]. For any pair of vertices in the graph, there is at most a
...
[ EK: Maybe we should right formally the connectivity cause it is not obvious ]
By construction, $\VD(G_d)=2^d+1$, so that
$\lfloor\log_2(\VD(G_d)-2)\rfloor+1=d$. The [$3(2^{d-1})$][ EK: "The $G_d$ concertina graph has total $3(2^{d-1})$ vertices, that can be partitioned in..."] vertices of $G_d$ can
...
\emph{corresponding bottom vertex}, i.e., the bottom vertex $u$ whose [ EK: "two" ] neighbors
are the same [ EK: "two" ] middle vertices that are neighbors of $v$. Among the middle
...
\begin{proof}
%\begin{IEEEproof}
  As we said, the $d$ paths in $Q$ [ EK: "will" we haven't constructed them yet] go from $v_\ell$ to $v_\mathrm{r}$.
  ...
  [$S=2^Q\setminus\{Q,\emptyset\}$][ EK: We haven't defined Q yet. 
  This definition looks like a number rather than set. It might be useful to do a sketch of the proof in three sentences and then
  proceed formally. 
  It seems like you want to relabel the elements according to the power-set without the two extremes.
  I understood your argument but it took a while. Maybe we can work on a simplified presentation.].
