...
between them, the VC-dimension of $\range_G$ collapses to a constant. This is a
somewhat surprising result with interesting consequences. From a theoretical
point of view, it points out that there are probably other characteristic
quantities of the graph different from the vertex diameter that control the
VC-dimension of the range set of shortest paths, and these quantities are a
constant on graph with a unique shortest path between vertices. From a more
practical point of view, we will see in the next section that this result has an
...
like road networks. In particular, the resulting sample size will be completely
independent from any characteristic of the network, and will only be a function
of the desired approximation guarantees.
...
%\end{IEEEproof}
\end{proof}
This bound is actually strict, as shown by the following lemma.
