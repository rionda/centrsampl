...
%\begin{IEEEproof}
  ...
  $S=2^Q\setminus\{Q,\emptyset\}$. We can
  ...
  we arbitrarily put $s'$ in $A$ and $s''$ in $B$. It is
  easy to see that $|A|=2^{d-1}-1$, which is the number of top
  vertices of $G_d$, and analogously for $|B|$ and the number of bottom vertices.

  We now build a map $\mathsf{r}$
  from the elements of $S$ to the set of top and bottom vertices of $G_d$.
