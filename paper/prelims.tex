...
that $G$ has a single connected component \XXX necessary? and that there are no self-loops and no multiple edges
...
as $\mathcal{S}_{uv}$. If $\sigma_{u,v}=0$, then
...
pair $(u,v)\in V\times V$ of distinct nodes: 
...
\XXX TBD: Define local version of betweenness centrality. Check papers.
