...
and $m=|E|$ edges. The graph $G$ can be directed or undirected. [ EK: "For this work," ] We assume that
there are no self-loops from any vertex to itself and no multiple edges between
...
$p_{uv}$}. The [ EK: "edge" ? and maybe mention that from that point and on we wil refer to 
the term as just weight.] \emph{weight}
...
in $G$][ EK: "is the number of vertices of the shortest path with the maximum size." We defined
size before]. If all the edge weights are unitary, then $\VD(G)$ is equal to
...
graph,][ EK: "Just by looking at the graph, one expects" don't think intuitive is the right word]
that vertices $\mathrm{b}$ and $\mathrm{g}$ should have higher
...
\begin{figure}[ht]
 ...
 % }
  ...
  [ EK: One expects to see a $v$ according to the notation $\betw(v)$. Maybe
  if we write "Vertex $v=$" in the first row of the table?]
...
\emph{shattered} by $\range$ if [ EK: I know that this notation $P_\range(B)=2^B$ is used for the definition of
of a shattered set, but it is a bit confusing. My problem is that even though it is clear that
$B$ is a set, it is not clear (at least to me) what is the $2^B$ term. Maybe if we add an explanatory sentence
on that...
]$P_\range(B)=2^B$.
...
[ EK: Is it possible to add a sentence on what is \mathds{1}_A(X_j)? some people might not be familiar with 
the notation. It is easy to infer, but ....]
\[
...
\begin{definition}\label{def:eapprox}
  ...
  an \emph{$\varepsilon$-approximation to $(\range,\phi)$} is a [ EK: By bag you mean unordered set?
  If yes, then whay not say a collection?] bag $S$ of
...
The constant $c$ is [approximately][ EK: "estimated to be (approximately)" 0.5 "according to"...] $0.5$~\citep{LofflerP09}.
