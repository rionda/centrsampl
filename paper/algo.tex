...
$k$-bounded-distance betweenness, the sample size can
be reduced to 
\[ 
...
If the graph is directed and/or is weighted, the computation of a good
...
as upper bound to $\VD(G)$. \XXX describe how we compute it. Although this
quantity can be as high as $n$, in the computation of the sample size we use its
logarithm. This has the effect of making our sample size comparable with that
proposed by~\citet{BrandesP07}, but the amount of work done per sample in our
algorithm is still much smaller, in practice, than that done by the algorithm
they presented (see Sect.~\ref{sec:discussion} and~\ref{sec:exper} for more
details).
