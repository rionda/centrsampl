...
deal with the latter, and then present a linear-time constant-factor
approximation algorithm for $\VD(G)$. %
...
as the exact betweenness algorithm by~\citet{Brandes01}. The \texttt{getDiameterApprox()} procedure computes an approximation for $\VD(G)$. % (see Sect.~\ref{sec:diam} for details).
...
%\begin{IEEEproof}
  ...
  \]
  where we used a result by~\citet[Lemma 3]{Brandes01} which gives us the
  following expression about the number of shortest paths for $w\neq u$,
  \[
...
\ref{alg:diamcomp} of Alg.~\ref{alg:algorithm}). Computing the exact value of
$\VD(G)$ could be done by solving the All Pair
...
%\begin{IEEEproof}
  ...
  shortest path between $v$ and $z$. From the properties of the shortest paths
  $p_{vu}$, $p_{vw}$ we have $|p_{vu}|+|p_{vw}|\geq |p_{vx}|+|p_{vz}|$. Since
  the graph is undirected $|p_{s,t}|=|p_{t,s}|$ for every $s,t\in V$. Therefore:
  \[
...
algorithm on each component, and then taking the maximum. The connected
components can be computed in $O(n+m)$ by traversing the graph in
...
size is comparable to that proposed by~\citet{BrandesP07}.\footnote{After this
work was accepted for publication, \citet{BergaminiM15} presented an improved
upper bound for $\VD(G)$ for undirected weighted graphs.} %when there is a
...
\]
In the second phase, we run Algorithm~\ref{alg:algorithm} with $r=r''$. Let
$\tilde\betw''_K$ be the so-obtained $K$-th highest estimated betweenness and
let $\ell''= \tilde\betw''_K / (1+\varepsilon)$. We return the collection
\[
	\widetilde\TOPK(K,G)= \left\{v\in V ~:~
\frac{\tilde\betw''(v)}{1-\varepsilon}\ge\ell''\right\}\enspace.
\]
\ifproof
...
\begin{algorithm}[ht]
  ...
  $\tilde\betw_K''\leftarrow$ $K$-th highest betweenness value from $B''$, ties
  broken arbitrarily\;
  $\ell''\leftarrow\betw_K''/(1+\varepsilon)$\;
  \Return $\{(v,\tilde\betw''(v)) ~:~ v\in V \mbox{ s.t. }
  \frac{\tilde\betw''(v)}{1-\varepsilon}\ge\ell''\}$\;
...
%similar to the original exact one.
...
We now compute $\exp[(\tilde\betw(w))^2]$. Consider the contribution $Y_i$ to
$\tilde\betw(w)$ of the $i$\textsuperscript{th} sampled path by Algorithm 1, for
$1\le i\le r$. $Y_i$ is a random variable that takes value
