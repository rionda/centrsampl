...
\paragraph{Unique shortest paths case} When there is an unique (or no) shortest
...
\emph{independent from $G$}. Unique shortest paths are common or even enforced
in road networks by slightly perturbing the edge weights or having a
deterministic tie breaking policy~\citep{GeisbergerSS08}.
...
Sect.~\ref{sec:discussion} and~\ref{sec:exper} for more details).
