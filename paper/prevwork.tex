...
[ EK: Need to work a bit on the flow of the next paragraph.]
...
[overshooting][ EK: seems like an informal term, but I don't have a better suggestion].
Our algorithm [is different from these because we sample][ EK: "takes a different approach from the above algorithms. Specifically"], each
...
\ifproof
[ EK: Need a better bridge with the previous sentence. "For certain applications it is crucial/enough to 
have a good centrality approximation of the top-K vertices."]
...
multiple ways][ EK: "multiple times," or on multiple rounds] by removing degree-1 vertices and identical vertices, and [splits it][ EK: "proceeds
by splitting the network"]
...
edges), but do not [deal with][ EK: "analyze/study the case of"]  shortest paths. \citet{AbrahamDFGW11} use
...
contribution is a tighter analysis [ EK: "and a (thorough) comparison"?] of the algorithm from~\citep{BrandesP07},
